...
const altezzaMassima = 80; 
const altezzaMinima = 70;  
const scrollMax = 200;     

window.addEventListener("scroll", () => {
  // Calcola quanto hai scrollato, massimo scrollMax
  const scrollY = Math.min(window.scrollY, scrollMax);
  // Calcola la nuova altezza in base allo scroll
  const nuovaAltezza = altezzaMassima - ((altezzaMassima - altezzaMinima) * (scrollY / scrollMax));
  // Applica la nuova altezza
  NAVBAR.style.height = `${nuovaAltezza}px`;
});
